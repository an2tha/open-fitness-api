...
export const nutrientsTable = pgTable(
  'nutrients',
  {
    ...
    })('search_vector').generatedAlwaysAs(sql`to_tsvector('english', "name")`),
  },
  (t) => ({
    nameUnitIdx: index('nutrients_name_unit_idx').on(t.name, t.unit),
...
export const foodNutrientsTable = pgTable(
  'food_nutrients',
  {
    ...
    nutrientId: integer()
      .notNull()
      .references(() => nutrientsTable.id, { onDelete: 'cascade' }),
