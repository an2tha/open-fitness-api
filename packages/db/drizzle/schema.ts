...
export const exerciseMovementPatterns = pgTable(
  ...
  (table) => [
    index('exercise_movement_patterns_pk').using(
      'btree',
      table.exerciseId.asc().nullsLast().op('int4_ops'),
      table.movementPatternId.asc().nullsLast().op('int4_ops'),
    ),
    foreignKey({
...
export const equipment = pgTable(
  ...
  (table) => [uniqueIndex('equipment_name_idx').using('btree', table.name.asc().nullsLast().op('text_ops'))],
);
...
export const exercises = pgTable(
  'exercises',
  {
    ...
    searchVector: unknown('search_vector').generatedAlwaysAs(
      sql`to_tsvector('english'::regconfig, (((name)::text || ' '::text) || (COALESCE(description, ''::character varying))::text))`,
    ),
  },
  (table) => [
...
export const exerciseMuscles = pgTable(
  ...
  (table) => [
    index('exercise_muscles_pk').using(
      'btree',
      table.exerciseId.asc().nullsLast().op('int4_ops'),
      table.muscleId.asc().nullsLast().op('int4_ops'),
...
export const prohibitedSubstances = pgTable('prohibited_substances', {
  id: integer().primaryKey().generatedAlwaysAsIdentity({
    name: 'prohibited_substances_id_seq',
    startWith: 1,
    increment: 1,
    minValue: 1,
    maxValue: 2147483647,
...
export const supplements = pgTable('supplements', {
  ...
  // TODO: failed to parse database type 'tsvector'
  searchVector: unknown('search_vector').generatedAlwaysAs(
    sql`to_tsvector('english'::regconfig, (((name)::text || ' '::text) || (COALESCE(brand, ''::character varying))::text))`,
  ),
});
...
export const exerciseEquipment = pgTable(
  ...
  (table) => [
    index('exercise_equipment_pk').using(
      'btree',
      table.exerciseId.asc().nullsLast().op('int4_ops'),
      table.equipmentId.asc().nullsLast().op('int4_ops'),
    ),
    foreignKey({
      columns: [table.exerciseId],
      foreignColumns: [exercises.id],
...
export const exerciseRelations = pgTable(
  ...
  (table) => [
    ...
    primaryKey({
      columns: [table.fromExerciseId, table.toExerciseId, table.relationType],
