...
const unzip = () => {
  const logger = getLogger();
  return Bun.file(paths.foodName)
    .exists()
    .then((exists) => {
      if (exists) return;
      logger.setProgress(DATA_SOURCE, 0, 1, 'extracting CNF');
      return $`mkdir -p ${EXTRACT_PATH}`
        .quiet()
...
const forEachCsvRow = async (
  ...
  for (let i = 0; i < content.length; i++) {
    ...
    if (onProgress && i - lastProgress >= 262144) {
      lastProgress = i;
      onProgress(i, total);
    }
  }
...
export const loadCnfFoods = async () => {
    ...
    (row, i) => {
      if (i) {
        const id = text(row[0]),
          unit = text(row[3]),
          name = text(row[4]);
        if (id && name) nutrients.set(id, { name, unit: unit ?? '' });
      }
    },
    progress,
  );
  ...
  await forEachCsvRow(
    paths.nutrientAmount,
    (row, i) => {
      if (!i) return;
      const food = foods.get(row[0] ?? ''),
        nId = row[1],
        val = row[2];
      if (!food || !nId || val === undefined) return;
      switch (nId) {
        case nutrientIds.calories:
          food.calories = decimal(val);
          break;
        case nutrientIds.kilojoules:
          if (food.calories === '0.00') food.calories = decimal(Number(val) * 0.239);
          break;
        case nutrientIds.protein:
          food.protein = decimal(val);
          break;
        case nutrientIds.fat:
          food.fat = decimal(val);
          break;
        case nutrientIds.carbohydrates:
          food.carbohydrates = decimal(val);
          break;
        case nutrientIds.fiber:
          food.fiber = decimal(val);
          break;
        case nutrientIds.sugar:
          food.sugar = decimal(val);
          break;
        case nutrientIds.sodium:
          food.sodium = decimal(val);
          break;
        default:
          food.rawNutrients.push({ id: nId, value: decimal(val) });
      }
    },
    progress,
  );
