...
const DOWNLOAD_URL = 'https://webapp.prod.blv.foodcase-services.com/wp-content/uploads/2025/07/Swiss_food_composition_database.xlsx';
...
export const loadSwissFoods = async () => {
  ...
  for (const sheetName of sheetsToProcess) {
    ...
    for (const key of nutrientKeys) {
      ...
      await db.insert(nutrientsTable)
        .values({ name, unit })
        .onConflictDoNothing();
      ...
      }).from(nutrientsTable).where(sql`${nutrientsTable.name} = ${name} AND ${nutrientsTable.unit} = ${unit}`).limit(1);
    ...
    for (let i = 0; i < rows.length; i += 500) {
      ...
      const insertedFoods = await db.insert(foodsTable).values(foodsToInsert).onConflictDoNothing().returning({ id: foodsTable.id, externalId: foodsTable.externalId });
