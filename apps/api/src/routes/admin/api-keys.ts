...
// OR by a valid better-auth session.
// Pass it via "Authorization: Bearer <master-key>" header, or include session cookie.
...
async function requireAdmin(c: Context, next: Next) {
  ...
  if (sessionToken) {
    try {
      ...
      // Session invalid, fall through to master key check
    }
  }

  // Fall back to master key check
  const authHeader = c.req.header('authorization');
  const provided = authHeader?.replace(/^Bearer\s+/i, '');
  if (!provided || provided !== masterKey) {
    throw new UnauthorizedError('Invalid master key or session.');
  }
